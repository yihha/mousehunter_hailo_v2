...
class TestFrameEvaluation:
    ...
    def test_evaluate_frame_bird_high_confidence(self, prey_detector):
        ...
        frame = DetectionFrame(
            timestamp=datetime.now(),
            detections=[cat, bird_low],
            frame_number=1,
            inference_time_ms=10.0,
        )

        result = prey_detector._evaluate_frame(frame)
        assert result.has_valid_prey is False  # Bird below threshold
        ...
        frame2 = DetectionFrame(
            timestamp=datetime.now(),
            detections=[cat, bird_high],
            frame_number=2,
            inference_time_ms=10.0,
        )

        result2 = prey_detector._evaluate_frame(frame2)
        assert result2.has_valid_prey is True  # Bird above threshold
